...
![image.png](https://s2.loli.net/2025/12/03/mT8Y7yNbIcRguXW.png)

场景：children之间的查找、插入都是o(n)。如果用有序，二分查找log(n),插入依然是o(n)。

设计：层叠红黑树。children之间的查找log(n),插入log(n)。

```js
层1：[红黑1-1]
层2：[红黑2-1,红黑2-2,红黑2-3......]
层3：[红黑3-1，红黑3-2.........]
`n`层某`红黑节点`，需指向`n+1`层某`根节点`。结合实际业务，不利于维护。但是效率拉满
```

2025-12-4，上面的应用就是一坨屎，可以用字典树去做，用map查询以及新增
