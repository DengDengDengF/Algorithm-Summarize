...
while(doubleBlack){
   ...
   if(parent.left === cur){
       bro=parent.right;
       if(!bro.color){//兄弟黑色
            if(bro.left && bro.left.color || bro.right && bro.right.color){ //孩子有一个红
             //调用3.2.1.1
            }else{//孩子全黑
             //3.2.1.2
            }
...
doubleBlack=parent;
stack.pop();
if(parent.color || stack.length===1){//遇到红或根变成单黑。
    parent.color=false;
    doubleBlack=null;
}
...
```js
//假设传递给该函数  双黑节点doubleBlack 和 它的父 parent 和 stack
let bro;
if(parent.left === doubleBlack){
     ...
     //左旋...
     stack.splice(stack.length-2,0,bro);
}else{
     bro=parent.left;
     bro.color=!bro.color;
     parent.color=!parent.color;
...
```js
let parent=stack[stack.lengh-2];
let del=stack[stack.length-1];
let doubleBlack=null;//双黑
if(del.left && !del.right){//只有左
      //2.1
}else if(del.right && !del.left){//只有右
      //2.2
}else if(del.left && del.right){//左右孩子都有
     
}else{//没有孩子
     if(del.color){ //没有孩子的红孩子，可以直接删除
        //3.1
     }else{//没有孩子的黑节点，重难点
         doubleBlack=del;//双黑节点标记
         let bro;
         if(parent.left === del){
             bro=parent.right;
             if(!bro.color){//没孩子的黑节点，它的兄弟黑色
                 //3.2.1
             }else{//没孩子的黑节点，它的兄弟红色
                 //3.2.2
             }
