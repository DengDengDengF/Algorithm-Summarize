...
```js
m阶，非根关键字最大m-1个节点;关键字最小Math.cel(m / 2)-1.
    根,关键字最小1个
btree的查询:为了找到合适的区间插入，小于往左查，大于往右查，形成查询栈
btree的添加:根据查询的区间，进行顺序插入，插入后判断节点是否小于最大关键字，否则，进行分裂，父是不是符合?向上判断！
btree的区间搜索:会沿着二叉树搜索
btree的删除：就是下溢出后，判断是否可借/合并，
...
m阶，非根关键字最大m-1个节点;关键字最小Math.cel(m / 2)-1.
    根,关键字最小1个
b+tree查询:和b树类似
b+tree添加:叶子节点超过最大关键字进行分裂，分裂点要向上移，满足>=右侧，影响了非叶子节点，父是不是符合？向上判断？当然叶子节点分裂也要考虑链表的插入。
b+tree删除:非叶子之间的下溢出合并/可借，copybtree的删除。叶子和非叶子之间可借/删除，要特殊处理，主要是添加了链表操作，还要判断删除的位置，如果是0要特殊处理....
btree的区间搜索:搜索到一个值，就可以顺着链表一直next,不需要沿着二叉树回溯。     
```
...
若高度为2，即"一个根节点 + 若干叶子节点",存储记录数=跟节点指针数 * 单个叶子节点记录行数。
         假设根节点主键id是BigInt长度8字节,指针6字节，
         一个索引，8+6=14字节。
         一个索引页有 16*1024=16384字节
         索引数为：16384 / 14=1170个
         一个索引页就有1170个索引。
         假设一行1k, 一页有16行。
         总共有1170 * 16 = 18720条
...
若高度为3，即"一个根节点 + 若干非叶子节点 + 若干叶子节点",存储记录数=跟节点指针数 * 单个叶子节点记录行数。
        1170 * 1170 * 16 =21902400条
高度为3，总共21902400条       


若高度为n,总共 1170^(n-1) * 16条
```
